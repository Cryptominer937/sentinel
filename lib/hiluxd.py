...
class HiluxDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some hiluxd methods
    ...
    @classmethod
    def from_hilux_conf(self, hilux_dot_conf):
        from hilux_config import HiluxConfig
        config_text = HiluxConfig.slurp_config_file(hilux_dot_conf)
        creds = HiluxConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = hiluxlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = hiluxlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = hiluxlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_hiluxd_proto_version_with_sentinel_ping)
