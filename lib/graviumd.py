...
class GraviumDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some graviumd methods
    ...
    @classmethod
    def from_gravium_conf(self, gravium_dot_conf):
        from gravium_config import GraviumConfig
        config_text = GraviumConfig.slurp_config_file(gravium_dot_conf)
        creds = GraviumConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = graviumlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = graviumlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = graviumlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_graviumd_proto_version_with_sentinel_ping)
