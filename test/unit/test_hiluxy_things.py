...
def test_valid_hilux_address():
    from hiluxlib import is_valid_hilux_address

    main = valid_hilux_address()
    test = valid_hilux_address('testnet')

    assert is_valid_hilux_address(main) is True
    assert is_valid_hilux_address(main, 'mainnet') is True
    assert is_valid_hilux_address(main, 'testnet') is False

    assert is_valid_hilux_address(test) is False
    assert is_valid_hilux_address(test, 'mainnet') is False
    assert is_valid_hilux_address(test, 'testnet') is True


def test_invalid_hilux_address():
    from hiluxlib import is_valid_hilux_address

    main = invalid_hilux_address()
    test = invalid_hilux_address('testnet')

    assert is_valid_hilux_address(main) is False
    assert is_valid_hilux_address(main, 'mainnet') is False
    assert is_valid_hilux_address(main, 'testnet') is False

    assert is_valid_hilux_address(test) is False
    assert is_valid_hilux_address(test, 'mainnet') is False
    assert is_valid_hilux_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from hiluxlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % hiluxlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(hiluxlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(hiluxlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(hiluxlib.blocks_to_seconds(16616)) == 2612035
